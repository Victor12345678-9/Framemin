<?php
    ...
    class Router{
        ...
        public function start(){
            if($this->status == false){
                ...
                $res->status(404)->send(array(
                    "message" => "La ruta del metodo ".$_SERVER['REQUEST_METHOD']." no existe en este directorio"
                ));
            }
        }
        ...
        public function get($root,$controll,$callback){
            $sanitized = $this->sanitizeRoots("GET", $root);
            if($sanitized != false){
                $req = $this->WriteRoot($root, $sanitized['regex'], $sanitized['uri']);
                if($this->input == false){
                    if($controll!=[]){
                        $this->content = include('app/controllers/' . $controll[0] .'.php');
                        $clss = new $controll[0]();
                        $method = $controll[1];
                        $result = $clss->$method($req['params']);
                    }
                    $callback(new Request($_GET, $req['params'], $req['headers'], $result), new Response);
                }else{
                    if($controll!=[]){
                    ...
                    $callback(new Request($this->input, $req['params'], $req['headers'], $result), new Response);
                }
            }
        }

        public function post($root, $controll, $callback){
            ...
            if($sanitized != false){
                ...
                if($this->input == false){
                    if($controll!=[]){
                        ...
                        $clss = new $controll[0]();
                        $method = $controll[1];
                        $result = $clss->$method($req['body']);
                    }
                    $callback(new Request($_POST, $req['params'], $req['headers'],$result), new Response);
                }else{
                    if($controll!=[]){
                        $this->content = include('app/controllers/' . $controll[0] .'.php');
                        $clss = new $controll[0]();
        ...
        public function put($root,$controll, $callback){
            ...
            if($sanitized != false){
                ...
                if($this->input == false){
                    ...
                    // $callback(new Request($_PUT, $req['params'], $req['headers'],$result), new Response);
                }else{
                    if($controll!=[]){
                        $this->content = include('app/controllers/' . $controll[0] .'.php');
                        $clss = new $controll[0]();
        ...
        public function patch($root,$controll, $callback){
            ...
            if($sanitized != false){
                ...
                if($this->input == false){
                    ...
                    $callback(new Request($_PATCH, $req['params'], $req['headers'],$result), new Response);
                }else{
                    if($controll!=[]){
                        $this->content = include('app/controllers/' . $controll[0] .'.php');
                        $clss = new $controll[0]();
        ...
        public function delete($root, $controll, $callback){
            ...
            if($sanitized != false){
                ...
                if($this->input == false){
                    ...
                    $callback(new Request($_DELETE, $req['params'], $req['headers'],$result), new Response);
                    print_r($callback);
                }else{
                    if($controll!=[]){
                        $this->content = include('app/controllers/' . $controll[0] .'.php');
